...
def now_parser() -> str:  # 1- yesterday 2 - today 3 - tomorrow 4 - now
    ...
    wind = (
        soup.findAll("div", class_="unit unit_wind_m_s")[0]
        .get_text()
        .replace("м/c", " м/с ")
    )
    pressure = (
        soup.findAll("div", class_="unit unit_pressure_mm_hg_atm")[0]
        .get_text()
        .replace("ммрт. ст.", "")
    )
    humidity = (
        soup.findAll("div", class_="now-info-item humidity")[0]
        .get_text()
        .replace("Влажность", "")
    )
    gm = (
        soup.findAll("div", class_="now-info-item gm")[0]
        .get_text()
        .replace("Г/м активность", "")
        .replace("баллаиз 9", "")
    )
    water = (
        soup.findAll("div", class_="now-info-item water")[0]
        .get_text()
        .split()[0]
        .replace("Вода", "")
    )
...
def today_tomorrow_parser(fl: int) -> str:
    ...
    tomorrow_desc = soup1.find(
        "div", "weathertab weathertab-block tooltip"
    ).get("data-text")

    tomorrow_response = str(soup1.findAll("div", class_="weathertab-wrap")[1])
    soup2 = bs(tomorrow_response, "html.parser")

    tomorrow_date = soup2.find(
        "div", class_=re.compile(r"date date-\d")
    ).get_text()
